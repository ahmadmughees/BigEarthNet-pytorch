...
def train(net, train_, val_, criterion, optimizer, epochs, scheduler=None, weights=None, save_epoch = 1):
    ...
    for e in range(1, epochs + 1):
        ...
        if e % save_epoch == 0:
            ...
            torch.save(net.state_dict(), '.\Big_Resnet18_withloss{}'.format(e+41))
    return net
def sampler_(labels):
    ...
    return sampler
def loader(BATCH_SIZE = 700,train_split=0.60):
...
if __name__ == '__main__':#https://discuss.pytorch.org/t/brokenpipeerror-errno-32-broken-pipe-when-i-run-cifar10-tutorial-py/6224/4
    ...
    #net = train(net, train_loader, val_loader, criteria, optimizer,25,scheduler)
